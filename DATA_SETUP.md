...
| `data/raw/Permohonan Data YBTM - KE/` | varies | Raw station rainfall data from JPS/DID |
...
| Multi-river 4 rivers | `Permohonan Data YBTM - KE/` folder | Johor, Kedah, Klang, Kuantan (53 stations) | Phases 4–7 (processed CSVs already in repo) |

> **Note:** The processed versions of the multi-river data (`johor_daily_raw.csv`, `kedah_daily_raw.csv`, `klang_daily_raw.csv`, `kuantan_daily_raw.csv`) are already in `data/processed/` and committed to the repo. You only need the raw files if you want to re-run the data cleaning step from scratch.
...
│   ├── era5_all_rivers_mapped.csv
│   ├── imerg_stations_monthly.csv
│   ├── climate_indices_daily.csv
│   └── ...
└── raw/
    ├── BulkExport-*.csv     ← Pahang original data
    └── Permohonan Data YBTM - KE/   ← multi-river raw data
```
